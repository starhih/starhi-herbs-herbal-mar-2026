...
async function seed() {
  ...
  // 1. Fetch probiotics category and update probiotics products
  ...
  if (probioticsCategory) {
    console.log('Seeding Probiotics products...');
  ...
  // 2. Fetch Storg parent and update child products
  const vitCategory = await payload.find({ 
    collection: 'categories', 
    where: { slug: { equals: 'vitamins-minerals' } } 
  });
  
  if (vitCategory.docs[0]) {
    console.log('Seeding Storg vitamins and minerals parent/children...');
    let storgParent = await payload.find({ 
      collection: 'products', 
    ...
    for (const c of storgChildren) {
      let existingChild = await payload.find({ 
        collection: 'products', 
      ...
      if (!existingChild) {
        existingChild = await payload.create({
          ...
          data: {
            ...
            productIndications: {
              indications: c.indications.map(ind => ({ name: ind }))
            }
          }
        });
      } else {
