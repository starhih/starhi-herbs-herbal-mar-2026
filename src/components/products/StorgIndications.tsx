...
interface StorgIndicationsProps {
  product: Product;
}

export default function StorgIndications({ product }: StorgIndicationsProps) {
  ...
  return (
    <div className="w-full">
      <h3 className="text-xl font-semibold text-[#214842] mb-6">Health Indications</h3>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {product.indications.map((indication) => (
          ...
          >
            <div className="text-[#258F67] mb-3">
              {indicationIcons[indication] || defaultIcon}
            </div>
            <span className="text-center text-sm font-medium text-[#214842]">{indication}</span>
          </div>
        ))}
      </div>
    </div>
